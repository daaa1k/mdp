...
LINUX_HASH=$(prefetch_binary "${BASE_URL}/mdp-linux-x86_64")
MACOS_HASH=$(prefetch_binary "${BASE_URL}/mdp-macos-aarch64")
...
FORMULA="HomebrewFormula/mdp.rb"
...
if gh repo view "$TAP_REPO" >/dev/null 2>&1; then
  ...
  cp "$FORMULA" "$TAP_DIR/Formula/mdp.rb"

  (
    cd "$TAP_DIR"
    git add Formula/mdp.rb
    git commit -m "chore: update mdp to ${TAG}"
  ...
  echo "  Then copy HomebrewFormula/mdp.rb to Formula/mdp.rb in the tap repo"
fi
